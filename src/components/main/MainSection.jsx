    ...
    <Col col={12} sm={6} md={4} lg={3}>
      <Card className="card overfolw-hidden mb-4 p-2"
        ...
        <Card.Body style={{ height: "25rem", overflow: "hidden" }}>
          <Card.Title  style={{ height: "3rem", overflow: "hidden" }}>{product.title}</Card.Title>
          <Card.Text style={{ height: "6rem", overflow: "hidden" }}>{product.description}</Card.Text>
          <Card.Text>${product.price}</Card.Text>
        <Button className="text-white" variant="warning">Add to cart</Button>
        </Card.Body>
      </Card>
    </Col>
  );
